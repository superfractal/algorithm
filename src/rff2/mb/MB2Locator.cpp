...
namespace merutilm::rff2 {
    ...
    std::unique_ptr<MB2Locator> MB2Locator::locateMinibrot(ParallelRenderState &state,
        ...
        if (state.interruptRequested() || !data.getReference() || !data.getPerturbator()) return nullptr;
        ...
        try {
            proposal = stms_bridge::locate(state,
                inputCenter.real.to_string(), inputCenter.imag.to_string(),
                data.fractalSettings.general.logZoom, period, data.fractalSettings.general.threads);
        } catch (const std::exception &e) {
            vkh::logger::log_err("Locate Minibrot: {}", e.what());
            return nullptr;
        }
        if (!proposal || state.interruptRequested()) return nullptr;
        auto view = located_view::fromProposal(data.fractalSettings, data.getPerturbator()->dcMax, *proposal);
